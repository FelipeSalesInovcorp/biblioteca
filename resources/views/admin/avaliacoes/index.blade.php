<x-app-layout>
    ...
    <div class="py-6">
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8 space-y-4">
            ...
            <div class="p-4 bg-white shadow rounded flex gap-2">
                <a class="px-3 py-1 rounded border {{ $estado==='suspensa'?'bg-gray-100':'' }}"
                    href="{{ route('admin.avaliacoes.index', ['estado'=>'suspensa']) }}">Pendentes</a>

                <a class="px-3 py-1 rounded border {{ $estado==='ativa'?'bg-gray-100':'' }}"
                    href="{{ route('admin.avaliacoes.index', ['estado'=>'ativa']) }}">Ativas</a>

                <a class="px-3 py-1 rounded border {{ $estado==='recusada'?'bg-gray-100':'' }}"
                    href="{{ route('admin.avaliacoes.index', ['estado'=>'recusada']) }}">Recusadas</a>
            </div>
            ...
            <div class="bg-white shadow rounded overflow-hidden">
                <table class="w-full text-sm">
                    <thead class="bg-gray-50">
                        <tr>
                            <th class="p-3 text-left">Requisição</th>
                            <th class="p-3 text-left">Livro</th>
                            <th class="p-3 text-left">Cidadão</th>
                        ...
                        <tr class="border-t">
                            <td class="p-3">
                                <a class="text-indigo-600 underline"
                                    href="{{ route('admin.avaliacoes.show', $a) }}">
                                    #{{ $a->requisicao->numero_sequencial ?? $a->requisicao_id }}
                                </a>
                            </td>
                            <td class="p-3">{{ $a->livro->nome ?? '—' }}</td>
                            <td class="p-3">{{ $a->user->name ?? $a->user->email }}</td>
                            <td class="p-3">{{ $a->classificacao }}/5</td>
