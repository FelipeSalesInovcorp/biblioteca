<x-app-layout>
    <x-slot name="header">
        <h2 class="text-2xl font-bold text-center  text-base-content leading-tight text-blue-800">✏️ Editar Editora</h2>
    </x-slot>

    <div class="py-8">
        <div class="max-w-3xl mx-auto">

            <div class="bg-base-100 p-6 rounded-box shadow space-y-6">

                @if ($errors->any())
                    <div class="alert alert-error">
                        <div>
                            <p class="font-semibold">Ocorreram erros ao guardar:</p>
                ...
                <form method="POST" action="{{ route('editoras.update', $editora) }}" enctype="multipart/form-data" class="space-y-5">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="form-control">
                        <label class="label">
                            <span class="label-text font-semibold">Logótipo (opcional)</span>
                        </label>

                        @if ($editora->logotipo)
                            <div class="mb-3 flex items-center gap-3">
                                <div class="avatar">
                                    <div class="w-16 rounded bg-base-200">
                                        <img src="{{ asset('storage/'.$editora->logotipo) }}" alt="Logótipo atual">
                                    </div>
                                </div>
                                <span class="text-sm text-base-content/70">Logótipo atual</span>
                            </div>
                        @endif
                        ...
                        <label class="label">
                            <span class="label-text-alt text-base-content/60">
                                Sugestão: PNG/SVG, fundo transparente, ~256×256.
                            </span>
                        </label>
                    </div>
