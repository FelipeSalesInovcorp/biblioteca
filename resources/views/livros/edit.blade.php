    ...
    <div class="py-8">
        <div class="max-w-3xl mx-auto bg-base-100 p-6 rounded-box shadow">
            <form method="POST"
                        ...
                        <select name="autores[]" class="select select-bordered w-full" multiple>
                            @foreach($autores as $autor)
                                <option value="{{ $autor->id }}"
                                    @if(collect(old('autores', $autoresSelecionados))->contains($autor->id)) selected @endif>
                                    {{ $autor->nome }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="grid gap-4 md:grid-cols-2">
                    <div>
                        <span class="label-text">Capa atual</span>
                        <div class="mt-2">
                            @if($livro->imagem_capa)
                                <div class="avatar">
                                    <div class="w-24 rounded">
                                        <img src="{{ asset('storage/'.$livro->imagem_capa) }}" alt="Capa {{ $livro->nome }}">
                                    </div>
                                </div>
                            @else
                ...
                <div class="form-control">
                    ...
                    <input type="number" step="0.01" min="0"
                           name="preco" value="{{ old('preco', $livro->preco) }}"
                           class="input input-bordered w-full" required>
                    @error('preco')
                        <span class="text-error text-sm">{{ $message }}</span>
                    @enderror
