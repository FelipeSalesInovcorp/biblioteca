<x-app-layout>
    <x-slot name="header">
        <h2 class="text-2xl font-bold text-blue-900 text-center">Novo Livro</h2>
    </x-slot>
            ...
            <form method="POST"
                ...
                <div class="grid gap-4 md:grid-cols-2">
                    <div class="form-control">
                        ...
                        <select name="editora_id" class="select select-bordered w-full" required>
                            ...
                            @foreach($editoras as $editora)
                                <option value="{{ $editora->id }}" @selected(old('editora_id') == $editora->id)>
                                    {{ $editora->nome }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-control">
                        ...
                        <select name="autores[]" class="select select-bordered w-full" multiple>
                            @foreach($autores as $autor)
                                <option value="{{ $autor->id }}"
                                    @if(collect(old('autores', []))->contains($autor->id)) selected @endif>
                                    {{ $autor->nome }}
                                </option>
                            @endforeach
                        </select>
                        <span class="text-xs text-base-content/70">Ctrl+Clique (ou Cmd+Clique) para selecionar múltiplos.</span>
                        @error('autores')
                            <span class="text-error text-sm">{{ $message }}</span>
                        @enderror
                    </div>
                ...
                <div class="form-control">
                    ...
                    <textarea name="bibliografia" rows="4"
                            class="textarea textarea-bordered w-full">{{ old('bibliografia') }}</textarea>
                    @error('bibliografia')
                        <span class="text-error text-sm">{{ $message }}</span>
                    @enderror
                </div>
